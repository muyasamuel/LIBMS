        ...
        if (res.status === 200) {
          ...
          const convertedBooks = books.map((book) => {
            return {
              ...book,
              issue_date:
                book.issue_date.slice(0, 16).replace(/T/g, " T") + " Hrs",
              return_date:
                book.return_date.slice(0, 16).replace(/T/g, " T") + " Hrs",
            };
          });
  ...
  const searchDate = (date) => {
    ...
    if (input !== "") {
      const filteredDates = issuedBooks.filter((item) => {
        return Object.values(item.return_date)
          .join("")
          .toLowerCase()
          .includes(input);
      });
      ...
      <div className="booksContainer">
        <div className="issuedBooksWrapper">
          <div className="heading">
            <h2> Filter to latest Return Date...</h2>
          </div>
          <div className="inputDiv">
            {" "}
            <input
              type="text"
              placeholder="Search by the return date.."
              value={input}
